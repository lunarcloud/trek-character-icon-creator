...
**Testing:** Playwright (accessibility specs), Jest (unit tests), and manual (see TESTING.md)
...
- **Use bullet point lists for file/directory tree structures** - File system hierarchies are clear and concise in hierarchical bullet list format.
...
The project has automated tests using Playwright (accessibility/browser specs) and Jest (unit tests), supplemented by manual testing.

1. **Before making changes:** Run existing app and tests to understand baseline
2. **After making changes:**
   - Run all linters (`npm run lint`)
   - Run automated tests (`npm test` and `npm run test:unit`)
...
- **Build:** Node.js build script (`node src/build-html.js`) generates `index.html` from `src/` partials
- **Dependencies:**
...
- **Dev Dependencies:**
  ...
  - Playwright with axe-core for accessibility tests
  - Jest for unit tests
...
- `/`
  - `index.html` - Main application UI (generated by build)
  ...
  - `src/` - HTML partials and build script
  ...
  - `tests/` - Playwright specs (`*.spec.js`) and Jest unit tests (`*.test.js`)
  - `fonts/` - Custom fonts
...
### Testing
- Automated accessibility tests using Playwright and axe-core: `npm test`
- Unit tests using Jest: `npm run test:unit`
...
- Steps: Install dependencies → ESLint → linthtml → stylelint → svglint → cspell → Validate Tooltips → Unit Tests
- All checks must pass for PR approval

**Accessibility Tests Workflow:** `.github/workflows/accessibility-tests.yml`
- Runs on: push to main, pull requests, manual trigger
- Steps: Install dependencies → Install Playwright browsers → Run Playwright tests
...
### Specialized Agents
The project defines specialized agents in `.github/agents/`:
- **code-quality** - Enforces coding standards, fixes linting issues, ensures JSDoc compliance
- **test-writer** - Creates and maintains Playwright specs and Jest unit tests
- **reviewer** - Reviews PRs against project standards and conventions
